...
app.post('/faculties', async (req, res) => {
  const { name, institution_name } = req.body; // Change institution_id to institution_name
  ...
  try {
    // Fetch institution ID using the institution name from the request body
    const institutionId = await getInstitutionId(institution_name);
    
    const sql = "INSERT INTO faculties (name, institution_id) VALUES (?, ?)";
    
    await query(sql, [name, institutionId]); 
    return res.status(201).json({ message: "Faculty added successfully." });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: err.message }); // Send specific error messages
  }
});
// Add a new course
